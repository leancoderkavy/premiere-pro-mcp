...
for (const variant of ["control", "test"]) {
  test(`${variant}: published repo facts, safe prompt, and actual setup destinations`, async ({ page, request }) => {
    ...
    await expect(page).toHaveTitle("Adobe Premiere Pro MCP | Free, Independent & Local")
    await expect(page.locator("h1")).toHaveText(variant === "test" ? /Adobe Premiere Pro MCP[\s\S]*Your assistant/ : /Adobe Premiere Pro MCP:/)
...
test("treatment: workflow chapters, Codex guide, manual setup, FAQs, and clipboard recovery", async ({ page, request }) => {
  ...
  await page.locator('a[href="/"]').first().click()
  await expect(page.locator("h1")).toHaveText(/Adobe Premiere Pro MCP[\s\S]*Your assistant/)
  ...
  const privacyFaq = page.getByRole("button", { name: "Does Premiere Pro MCP upload my footage or project files?", exact: true })
  await privacyFaq.click()
  await expect(privacyFaq).toHaveAttribute("aria-expanded", "true")
  const privacyAnswer = page.locator(`[id="${await privacyFaq.getAttribute("aria-controls")}"]`)
  await expect(privacyAnswer).toBeVisible()
  await expect(privacyAnswer).toContainText("No project media is automatically uploaded.")
  await expect(privacyAnswer).toContainText("Your AI assistant's separate privacy settings still apply to the prompts you send.")
...
for (const [label, headers] of [["DNT", { DNT: "1" }], ["GPC", { "Sec-GPC": "1" }], ["crawler", { "User-Agent": "Googlebot" }]] as const) {
  ...
  test(`${label}: no assignment, provider evaluation, or conversion`, async ({ page, context, request }) => {
    await setVariant(request, "test")
    await page.goto("/")
    await expect(page.locator("h1")).toHaveText(/Adobe Premiere Pro MCP:/)
    await copyButton(page).click()
