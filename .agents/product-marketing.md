...
**Release facts:** v1.14.6 registers 328 core tools; the default profile exposes 326; an authenticated compatible UXP host can add 80 capability-gated tools for a 406-tool connected surface. The release also declares 37 modules, 4 MCP resources, and 11 workflow prompts. It adds guarded UXP sequence, marker, transition, caption, and silence-removal workflows alongside bounded read-only Project-panel metadata inspection, guarded Project-panel metadata replacement with exact readback, guarded typed Project metadata schema-field creation with non-field-level readback, guarded named application-preference changes, guarded source-transcript replacement with exact export readback, guarded source-media frame-rate/pixel-aspect overrides with effective-value readback, direct active-track-item identity readback, guarded source-only timeline slips, guarded contiguous three-item timeline slides, guarded append-only timeline duplicates, and guarded contiguous same-track ripple deletes. These are catalog, packaging, and HTTP authorization facts from the repository, not a promise that a particular host operation will work.
