...
**Release facts:** v1.13.0 registers 313 core tools; the default profile exposes 311; an authenticated compatible UXP host can add 50 capability-gated tools for a 361-tool connected surface. The release also declares 36 modules, 4 MCP resources, and 11 workflow prompts. These are catalog and packaging facts from `release-metadata.json`, not a promise that a particular host operation will work.
