...
**Release facts:** v1.14.6 registers 328 core tools; the default profile exposes 326; an authenticated compatible UXP host can add 70 capability-gated tools for a 396-tool connected surface. The release also declares 37 modules, 4 MCP resources, and 11 workflow prompts. It adds guarded UXP sequence, marker, transition, caption, and silence-removal workflows alongside bounded read-only Project-panel metadata inspection, local-only delivery, and editorial review evidence. These are catalog, packaging, and HTTP authorization facts from the repository, not a promise that a particular host operation will work.
