...
| `inspect_premiere_events_uxp` | `events.list`, `events.wait` | `EventManager` and six root `SnapEvent.EVENT_SNAP_*` constants | Read-only bounded event receipt monitoring | Base event journaling remains capability-gated; SnapEvent notifications additionally require each named stable root constant to probe as a non-empty event name | Register only available documented SnapEvent constants as passive `timeline.snap.*` receipts. Return the ordinary 256-entry/60-second bounded journal with allowlisted scalar detail only; no raw host event payload, guaranteed emission, project-state invalidation, downstream edit completion, or licensed-host proof is claimed. |
