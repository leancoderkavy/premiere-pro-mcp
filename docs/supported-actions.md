...
| `inspect_sequence_timing_uxp` | Connected UXP | Single operation | Read the active sequence's native frame size, timebase, audio/video time-display codes, and backing Project-item identity. The command rejects malformed host values and an active-sequence switch during the read; it does not modify Premiere or claim a locked snapshot. |
