...
| Authenticated UXP additions | 90 | Advertised only while a compatible authenticated UXP panel is connected |
| Default profile with UXP | 416 | 326 core plus 90 UXP tools |
...
| `inspect_premiere_events_uxp` | Connected UXP | `list`, `wait` | List or briefly wait for bounded, redacted Premiere host-event receipts without polling the complete project state. Compatible hosts can also emit timeline.snap.* receipts for documented SnapEvent notifications; raw event payloads are never returned. |
