...
export const WORD_TIMELINE_PARAMETER = {
  ...
  description:
    "Caller-supplied word-timed transcript for one Premiere source item. Words must be ordered by start time and bound to the transcript revision returned by get_clip_transcript_uxp. Different labeled speakers may overlap; same-speaker words may not.",
  properties: {
...
export function validateWordTimeline(input: unknown, options: { maxWords?: number } = {}): WordTimeline {
  ...
  raw.words.forEach((entry, index) => {
    ...
    const nextSpeaker = typeof word.speaker_label === "string" ? word.speaker_label.trim() : undefined;
    const overlappingSpeakers = Boolean(previousSpeaker && nextSpeaker && previousSpeaker !== nextSpeaker);
    if (!overlappingSpeakers && start + OVERLAP_TOLERANCE_SECONDS < previousEnd) fail(`${label} overlaps the previous word by more than ${OVERLAP_TOLERANCE_SECONDS} seconds`);
